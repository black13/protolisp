...
start:
    ...
    if (NOT (DIGIT(c) OR ((c EQ PLUS OR c EQ MINUS) AND (DIGIT(lookgchar()) OR lookgchar() EQ DOT))))
    {
        np= nc; 
        *np++= c;    /* put c in nc[0] */
        for (c= lookgchar();c!=BLANK AND c!=DOT AND c!=OPENP AND c!=CLOSEP; c= lookgchar())
...
-----------------------------------------------------------------------------*/
{
    ...
    s[i]= '\0';
    if (c EQ EOF AND i EQ 0) return(-1); 
    else return(i);
}
 ...
 ----------------------------------------------------------------------------*/
{
    ...
    {
        gc(); 
        if (nf<0) error("The number table is full");
    }
    nx[j]= nf; 
    j= nf; 
    nf= Ntab[nf].nlink; 
...
// DEBUG(printf("ordatom: `%s' hashes to %d. k=%d, n=%d\n",s,j,k,n););
    ...
    {
        if (strcmp(Atab[j].name,s) EQ 0) 
            goto ret;
        else if (++j >= n) 
        {
            j= 0; 
            if (++c>1) error("atom table is full");
        }
    }
